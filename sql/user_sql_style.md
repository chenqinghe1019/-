...
适用于所有项目的生命周期、LTV、留存、付费、活跃、关卡、阵容、活动周期等分析：

- 先按创角日期、开服日期或活动起始日期圈选 cohort，日期筛选作用于样本起始日期，不直接截断后续事件。
- 观察第 0～N 天或一个完整周期时，事件表 `$part_date` 必须按每个样本从起始日自动延展至 `起始日+N`，不能依赖独立事件日期筛选器手工补范围。
- 只统计完整观察窗口已成熟的样本：`date_add('day',N,起始日期) <= 昨天`。
- 未成熟的新增日期、区服或活动周期必须剔除或将对应指标置空，禁止把不完整累计值当成完整指标。
- D7 表示 D0～D6，成熟条件固定为 `起始日期+6天 <= 昨天`，事件扫描范围固定为起始日期至起始日期+6天。
- 同一张分日对比表如要求一致 cohort，必须按最大观察天数统一筛选成熟样本，避免各天分母因成熟度不同而变化。
