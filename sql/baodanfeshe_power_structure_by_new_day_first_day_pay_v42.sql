...
-- 7. 每个“新增天数 × 首日付费分层”增加一条分层汇总，不跨付费分层合并。
-- 8. 正向战力贡献占比 = 该变化原因正向战力提升 / 同新增天数、同首日付费分层全部明细原因正向战力提升。
-- 9. 净战力贡献占比 = 该变化原因净战力变化 / 同新增天数、同首日付费分层全部明细原因净战力变化；存在负向变化时占比可为负或大于1，但同层明细合计为1（分母非0时）。
-- 10. 分层汇总行的正向战力贡献占比、净战力贡献占比均固定为1。
-- 11. 仅统计 current_date 之前完整日期，避免当天数据未完整。
