...
- 次日流失关卡分布口径：新增日期用 `create_role_time` 转日期并支持 `${PartDate:date}` 筛选；分层按首日 `pay_log.payment/100`；次日存在 `in_out_log` 视为留存，否则流失；最终停留关卡按首日 `battle_star` + `battle_result` 最大 `map_id`；没有主线记录时关卡 ID 记为 0；最终停留关卡是否通过只看首日同关卡 `battle_result`，没有 `battle_result` 认为已通过，`battle_result=1` 认为已通过，只有 `battle_result=0` 判断未通过。
