...
- 创角生命周期分日分析固定使用“自动延展 + 成熟样本”口径：先按 `date(create_role_time)` 圈选创角 cohort；观察 `days=0～N` 时，事件表 `$part_date` 必须按每个玩家从创角日自动延展至 `创角日+N`，不得再用独立事件日期筛选器截断；仅纳入 `date_add('day', N, date(create_role_time)) <= date_add('day', -1, current_date)` 的玩家，保证完整观察窗口已经成熟。D7 分析中 `N=6`，即只统计已完整经历 D0～D6 的玩家。
