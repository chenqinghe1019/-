...
- 英雄获得事件：`role_obtain_log`，英雄字段 `role_id`，但当前实际上报为英雄名；狐妖妲己筛选写 `cast("role_id" as varchar) = '狐妖妲己'`；初始星级字段 `init_star`。
- 英雄升星事件：`role_upstar_log`，英雄字段 `role_id`，但当前实际上报为英雄名；狐妖妲己筛选写 `cast("role_id" as varchar) = '狐妖妲己'`；新星级字段 `nstar`，原星级字段 `ostar`。
