...
-- 日期：2026-08-24
-- 口径：
-- 1. 查询周期由 ${PartDate:date} 控制。
-- 2. 按自然日分组；每天发生 in_out_log 的活跃角色作为当天总体。
-- 3. 获取 = change_type = 1；若 change_type 为空，则 item_num > 0 作为兜底。
-- 4. 当天活跃但未获取某道具的玩家按 0 补齐后，再计算人均、最小值、P25、P50、P75、P95、最大值。
-- 5. 获取率 = 获取玩家数 * 1.0000 / 活跃玩家数。
-- 6. 超出P95两倍玩家数 = 当天该道具获取数量 > 当天该道具P95获取数量 * 2 的活跃玩家数。
-- 7. P95沿用全体活跃玩家口径，未获取玩家按0参与；若P95=0，则所有获取数量>0的玩家都会满足>2*P95。
                    ...
                    (
                        SELECT
                            ...
                            coalesce(
                                ...
                                CASE
                                    WHEN try_cast(e."item_num" AS double) > 0
                                        THEN 1
                                    WHEN try_cast(e."item_num" AS double) < 0
                                        THEN 2
                                    ELSE 0
