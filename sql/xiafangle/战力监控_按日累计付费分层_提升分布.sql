-- 下方了｜按日期 × 开服天数 × 截至当天累计付费分层 战力绝对提升监控
-- 口径：
-- 1. ${PartDate:date} 用于限定统计周期，日期作为最终展示/分组维度。
