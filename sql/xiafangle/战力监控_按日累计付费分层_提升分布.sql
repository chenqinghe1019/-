...
-- 4. 当天战力提升 = 当天 change_power_log 最后一条 after - 第一条 before。
-- 5. 当天活跃但没有有效 change_power_log 的角色，当天战力提升按 0。
-- 6. change_power_log 可能存在后台重算，因此只在 in_out_log 活跃日纳入统计。
...
(
    select      t."日期",
                ...
                t."分层排序",
                count(*) "活跃玩家数",
                sum(case when t."当天战力提升" <> 0 then 1 else 0 end) "战力有变动玩家数",
                sum(case when t."当天战力提升" > 0 then 1 else 0 end) "战力提升玩家数",
                avg(cast(t."当天战力提升" as double)) "平均战力提升",
                min(cast(t."当天战力提升" as double)) "最小战力提升",
                approx_percentile(cast(t."当天战力提升" as double),0.25) "P25战力提升",
                approx_percentile(cast(t."当天战力提升" as double),0.50) "P50战力提升",
                approx_percentile(cast(t."当天战力提升" as double),0.75) "P75战力提升",
                max(cast(t."当天战力提升" as double)) "最大战力提升"
    from
    (
        select      s."日期",
                    s."#account_id",
