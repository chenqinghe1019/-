-- 步步 v22：渠道100纯新/服务器维度后续N日付费
-- 时间范围：2026-07-01 ~ 2026-08-15
-- 口径：
-- 1. 用户表渠道：cast(channel_id as varchar)='100'。
-- 2. 纯新：同一 nb_open_id 下 create_role_time 最早的角色；滚服角色剔除。
-- 3. 服务器维度：按 server_open_time 日期 + region_id 分组，只取开服当天创建角色，不剔除滚服。
-- 4. create_role_time 为 Unix 秒，使用 from_unixtime(try_cast(... as double))。
-- 5. pay_log.payment 单位为分，/100 转元；仅统计 payment>0。
-- 6. D1=创角当天/开服当天；输出 day_no 为纯数字。
-- 7. 本版本仅输出后续N日付费，不再输出付费留存。
