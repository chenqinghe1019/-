    ...
    (
        ...
        (
                ...
                max(
                    CASE
                        WHEN e."$part_event" = 'log_in_out'
                         AND date(e."#event_time")
                             BETWEEN date_add('day', 11, user_base."开服日期")
                    ...
                    CASE
                        WHEN e."$part_event" = 'pay_log'
                         ...
                         AND try_cast(e."product_id" AS bigint) IN
                             (20013, 20015, 20018, 20021)
                            THEN 1
                        ELSE 0
                    END
                ) AS "活动付费标记",

                /* 固定商品价格，不读取pay_log.payment */
                sum(
                    CASE
                        WHEN e."$part_event" = 'pay_log'
                         AND date(e."#event_time")
                            ...
                            CASE try_cast(e."product_id" AS bigint)
                                WHEN 20013 THEN 1200  / 100.0000
                                WHEN 20015 THEN 6800  / 100.0000
                                WHEN 20018 THEN 19800 / 100.0000
                                WHEN 20021 THEN 64800 / 100.0000
                                ELSE 0
                            END
                        ELSE 0
                    END
            ...
            (
                ...
                /* 只保留D12~D17完整落入统计范围的成熟区服 */
                WHERE date_add('day', 11, u0."开服日期")
                        >= stats_period."统计开始日期"
                  AND date_add('day', 16, u0."开服日期")
                        <= stats_period."统计结束日期"
            ) user_base
